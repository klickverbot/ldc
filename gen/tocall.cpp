...
//         Logger::cout() << "callable: " << *callable << '\n';
    ...
    {
        bool const appendVarargs = VarargABI::target()->lowerArgumentList(
            tf, callableTy, arguments, args.size(), args, attrs);

        // We gave the vararg ABI the chance to apply any special rewrites; now
        // handle a call to a "normal" function (might have plain C variadic
        // parameters).
        ...
        // do C varargs
        if (appendVarargs && n_arguments > n)
        {
